...
var testPassed = helper.testPassed;
...
describe('2 The then method', function () {
    ...
    describe('2.1 Both onFulfilled and onRejected are optional arguments', function (done) {
        ...
        it('2.1.2 If onRejected is not a function, it must be ignored.', function () {
            ...
            Promise.reject(error).then().then(preventOnFulfilled, function (reason) {
    ...
    // 2.4
    describe('2.4 onFulfilled or onRejected must not be called until the execution context stack contains only platform code.', function () {
    ...
    // 2.5
    describe('2.5 onFulfilled and onRejected must be called as functions (i.e. with no this value).', function () {
        it('onFulfilled this value', function (done) {
            Promise.resolve(1).then(function () {
                assert.ok((this === undefined) || (this === global));
            }).then(testPassed(done), testFailed(done));
        });
    ...
    describe('2.7 then must return a promise', function () {
        ...
        describe('2.7.1 If either onFulfilled or onRejected returns a value x, run the Promise Resolution Procedure [[Resolve]](promise2, x).', function () {
            ...
            it('onRejected returns a rejected promise', function (done) {
                var error = new Error('rejected');
                Promise.reject(new Error('some reason')).then(preventOnFulfilled, function () {
                    return Promise.reject(error);
                }).then(preventOnFulfilled, function (reason) {
            ...
            it('onFulfilled returns an eventually fulfilled thenable object being resolved multiple times', function (done) {
                var p = Promise.resolve(0).then(function () {
                    var w = new Thenable(function () {
                        setTimeout(function () {
                            w.resolve(1);
                            setTimeout(function () {
                ...
                p.then(function (value) {
                    ...
                    assert.equal(value, 1, 'then promise is resolved with eventually multiply fulfilled thenable returned by onFulfilled');
                }).then(testPassed(done), testFailed(done));
            });
            ...
            it('onFulfilled returns an eventually multiply rejected thenable object', function (done) {
                var error = new Error('rejected');
                var error2 = new Error('another error');
                var p = Promise.resolve(0).then(function () {
                    var w = new Thenable(function () {
                        setTimeout(function () {
                            w.reject(error);
                ...
                p.then(preventOnFulfilled, function (reason) {
                    assert.equal(reason, error, 'then promise is resolved with eventually rejected thenable returned by onFulfilled');
                    return Promise.delay(50)
                        .then(function () {
                            return p;
                        ...
                        .then(preventOnFulfilled, function (err) {
                            assert.equal(err, error, 'then promise is resolved with eventually rejected thenable returned by onFulfilled');
                        });
                }).then(testPassed(done), testFailed(done));
            });
