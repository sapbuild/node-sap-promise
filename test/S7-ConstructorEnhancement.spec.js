...
describe('7 Promise constructor enhancements', function () {
        ...
        it('should stand as a shortcut for Promise.resolve().delay()', function (done) {
            ...
            Promise.delay(15)
                .then(function (result) {
                    ...
                    assert.operator(Date.now(), '>=', dt + 14);
    ...
    describe('7.4 Promise.invoke', function () {
        ...
        it('should return a rejected promise in case of exception', function (done) {
            var error = new Error('I am a bad function');
            var obj = {
                bad: function () {
                    throw error;
                }
    ...
    describe('7.5 Promise.objectInvoke', function () {
        it('requires a string or a function as second argument', function (done) {
            Promise.invoke({})
                .then(preventOnFulfilled, function (reason) {
                    assert.ok(reason instanceof TypeError, 'requires a string or a function as second argument');
                    return Promise.invoke({}, {});
                })
                .then(preventOnFulfilled, function (reason) {
                    assert.ok(reason instanceof TypeError, 'requires a string or a function as second argument');
        ...
        it('should convert a callback-based object API into a promise-based one', function (done) {
            ...
            // Promise.objectInvoke with method name as 2nd parameter
            promises.push(Promise.objectInvoke(obj, 'echoArgs').then(function (result) {
                assert.equal(result, obj);
            }));
            promises.push(Promise.objectInvoke(obj, 'echoArgs', 1).then(function (result) {
                assert.deepEqual(result, [ obj, 1 ]);
            }));
            promises.push(Promise.objectInvoke(obj, 'echoArgs', 1, 2).then(function (result) {
                assert.deepEqual(result, [ obj, 1, 2 ]);
            }));
            promises.push(Promise.objectInvoke(obj, 'echoArgs', 1, 2, 3).then(function (result) {
                assert.deepEqual(result, [ obj, 1, 2, 3]);
            }));
            promises.push(Promise.objectInvoke(obj, 'echoArgs', 1, 2, 3, 4).then(function (result) {
                assert.deepEqual(result, [ obj, 1, 2, 3, 4 ]);
            }));
            promises.push(Promise.objectInvoke(obj, 'echoArgs', 1, 2, 3, 4, 5).then(function (result) {
                assert.deepEqual(result, [ obj, 1, 2, 3, 4, 5 ]);
            }));
            promises.push(Promise.objectInvoke(obj, 'fail').then(preventOnFulfilled, function (reason) {
                assert.equal(reason, error);
            }));

            // Promise.invoke with function as 2nd parameter
            promises.push(Promise.objectInvoke(obj, obj.echoArgs).then(function (result) {
                assert.equal(result, obj);
            }));
            promises.push(Promise.objectInvoke(obj, obj.echoArgs, 1).then(function (result) {
                assert.deepEqual(result, [ obj, 1 ]);
            }));
            promises.push(Promise.objectInvoke(obj, obj.echoArgs, 1, 2).then(function (result) {
                assert.deepEqual(result, [ obj, 1, 2 ]);
            }));
            promises.push(Promise.objectInvoke(obj, obj.echoArgs, 1, 2, 3).then(function (result) {
                assert.deepEqual(result, [ obj, 1, 2, 3]);
            }));
            promises.push(Promise.objectInvoke(obj, obj.echoArgs, 1, 2, 3, 4).then(function (result) {
                assert.deepEqual(result, [ obj, 1, 2, 3, 4 ]);
            }));
            promises.push(Promise.objectInvoke(obj, obj.echoArgs, 1, 2, 3, 4, 5).then(function (result) {
                assert.deepEqual(result, [ obj, 1, 2, 3, 4, 5 ]);
            }));
            promises.push(Promise.objectInvoke(obj, obj.fail).then(preventOnFulfilled, function (reason) {
                assert.equal(reason, error);
            }));
            Promise.all(promises).then(function () {
                done();
    ...
    describe('7.6 Promise.fnCall', function () {
        it('requires an object or function as first argument', function (done) {
            Promise.invoke()
                .then(preventOnFulfilled, function (reason) {
